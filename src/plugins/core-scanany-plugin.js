		...
		*/
		{
			...
			_execute: async (command, context) => {
				...
				for(let j=0; j < collection.length; j++){
					...
					let ctx = extend({}, context)
					ctx[command.as] = item
					ctx[command["indexed-by"]] = j
					
					let applyedCommands = command.apply.map( d => deepExtend({},d)) 

					for(let i=0; i< applyedCommands.length; i++){
						// console.log("data index",ctx[command["indexed-by"]], "command", applyedCommands[i])
						ctx = await scraperInstance.executeOnce(applyedCommands[i], ctx)
					}
					value = deepExtend({}, ctx)
					mapped.push(value)
					// delete context.$item
